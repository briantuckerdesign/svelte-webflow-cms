...
export async function loadReferenceData(
	...
	const referenceFields = tableConfig.fields.filter(
		(field) =>
			(field.schema.type === 'Reference' || field.schema.type === 'MultiReference') &&
			field.schema.validations?.collectionId
	);

	// Get unique collection IDs
	const collectionIds = [
		...new Set(referenceFields.map((field) => field.schema.validations.collectionId))
	];
	...
	await Promise.all(
		collectionIds.map(async (collectionId) => {
			try {
				...
				console.error(`Failed to fetch reference data for collection ${collectionId}:`, err);
