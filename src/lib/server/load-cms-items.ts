...
export async function loadCmsItems(token: string, tableConfig: TableConfig) {
	...
	try {
		const response = await client.collections.items.listItems(tableConfig.collectionId);
		...
		if (tableConfig.sortField) {
			items.sort((a, b) => {
				const aVal =
					(a.fieldData as Record<string, any>)?.[tableConfig.sortField!.schema.slug] ?? 0;
				const bVal =
					(b.fieldData as Record<string, any>)?.[tableConfig.sortField!.schema.slug] ?? 0;
