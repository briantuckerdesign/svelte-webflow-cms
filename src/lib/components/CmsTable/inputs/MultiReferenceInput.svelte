<script lang="ts">
  ...
  let availableItems = $derived(
    sortedItems.filter((item) => !items.includes(item.id))
  );
...
<div class="relative w-full" bind:this={containerRef}>
  <InputGroup.Root class="flex-nowrap gap-1 overflow-x-auto px-2 py-1">
    {#each selectedItems as item (item.id)}
      <Badge variant="secondary" class="shrink-0 gap-1 py-0 pr-1.5 text-sm">
        {item.name}
        {#if !disabled}
          <button
            type="button"
    ...
    <input
      ...
      class="min-w-10 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground"
