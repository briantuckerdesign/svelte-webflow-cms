<script lang="ts">
	...
	let { item = $bindable(), config, referenceData = {}, onDelete, onTempFile }: Props = $props();
...
{#if config.sortField}
	<Table.Cell class="w-10 cursor-grab px-4 py-3 text-gray-400 active:cursor-grabbing">
...
{#each config.fields as field}
	{#if field.visible}
		<Table.Cell
			class="px-1 py-3 align-middle"
			style="text-align: {field.styles?.align ?? fieldStyleDefaults[field.schema.type].align};"
		>
			<CmsCell
				{field}
				...
				<AlertDialog.Content>
					<AlertDialog.Header>
						...
						<AlertDialog.Description>
							This will mark this item for deletion. The change will be applied when you save.
						</AlertDialog.Description>
					</AlertDialog.Header>
					<AlertDialog.Footer>
