<script lang="ts">
	...
	let displayValue = $state(
		...
		schema.type === 'VideoLink' && typeof value === 'object' && value?.url ? value.url : value
	);
...
{:else if schema.type === 'Link' || schema.type === 'VideoLink'}
	...
	{:else}
		<a href={displayValue} class="text-sm text-blue-600 hover:underline" target="_blank"
			>{displayValue || '-'}</a
		>
	{/if}
{:else if schema.type === 'Email'}
	...
	{:else}
		<a href="mailto:{displayValue}" class="text-sm text-blue-600 hover:underline"
			>{displayValue || '-'}</a
		>
	{/if}
...
{:else if schema.type === 'Option'}
	{#if editable}
		<OptionInput bind:value={displayValue} options={schema.validations?.options || []} />
	{:else}
		{@const options = schema.validations?.options || []}
		<span class="text-sm"
			>{options.find((o: any) => o.id === displayValue)?.name || displayValue || '-'}</span
		>
	{/if}
{:else if schema.type === 'Reference'}
	{#if editable}
		{@const collectionId = schema.validations?.collectionId}
		{@const referenceItems = collectionId ? referenceData[collectionId] || [] : []}
		<ReferenceInput bind:value={displayValue} {referenceItems} />
	{:else}
		{@const collectionId = schema.validations?.collectionId}
		{@const referenceItems = collectionId ? referenceData[collectionId] || [] : []}
		{@const referencedItem = referenceItems.find((item: any) => item.id === displayValue)}
		<span class="text-sm">{referencedItem?.fieldData?.name || displayValue || '-'}</span>
	{/if}
{:else if schema.type === 'MultiReference'}
	...
	{:else}
		...
		{@const selectedNames = (displayValue || [])
			.map((id: string) => referenceItems.find((item: any) => item.id === id)?.fieldData?.name || id)
			.join(', ')}
